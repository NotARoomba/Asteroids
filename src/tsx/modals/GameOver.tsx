    ...
    >
      ...
      {hs ? (
        <div>
          ...
          <input className={'justify-center m-auto text-neutral-200 placeholder:text-neutral-400 bg-neutral-900 outline-double rounded focus:outline-double focus:outline-white py-1 text-center w-1/2' + (typing?' animate-shake':'')}  id='nameText' placeholder='Enter a username' value={n} onChange={handleTextChange} />
      ...
      <div className='flex flex-col justify-around text-center'>
        ...
        <table className='table-auto'>
            ...
            {data?.map((score, i) => {
                ...
                <td className={score.name == n&&score.score==curData.score?'animate-bouncepulse':''}>{i + 1}</td>
                <td className={score.name == n&&score.score==curData.score?'animate-bouncepulse':''}>{score.score}</td>
                <td className={score.name == n&&score.score==curData.score?'animate-bouncepulse':''}>{score.name}</td>
                </tr>
            })}
            </tbody>
        </table>
