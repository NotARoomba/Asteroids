...
export default function Home() {
  ...
  return (
    ...
    <Stage width={width} height={height} options={{backgroundColor: 0x000}} className="absolute -z-50 w-screen h-screen top-0 left-1/2 right-1/2 -translate-x-1/2">
        <Game count={10} screen={new vec2(width, height)} player={false} setScore={()=>1} setLevel={()=>1}/>
    </Stage>
    <div className='flex flex-col h-screen text-8xl bg-transparent text-neutral-200'>
      ...
      <Link to='/play' className='text-4xl m-auto justify-center align-middle text-center mt-5 py-4 px-10 outline-double outline-4 hover:animate-colorpulse'>Play</Link>
    </div>
    </Transitions>
  )
}
