...
export default function Play() {
    ...
    <Transitions>
        ...
        <Stage width={width} height={height} options={{backgroundColor: 0x000}} className="absolute -z-50 w-screen h-screen top-0 left-1/2 right-1/2 -translate-x-1/2">
            <Game count={4} screen={new vec2(width, height)} player={true} setScore={sScore} setLevel={sLevel} gameOver={gameOver}/>
        </Stage>
        {done?<Transitions><div className='flex flex-col h-screen text-neutral-200'><p className='align-middle justify-center m-auto mb-10 bg-black rounded animate-bouncepulse bg-opacity-50 text-8xl'>Game Over!</p><button onClick={() => window.location.reload()} className='text-4xl m-auto justify-center align-middle text-center mt-5 py-4 px-10 outline-double outline-4 hover:animate-colorpulse'>Play Again</button></div></Transitions>:<></>}
    </Transitions>)
}