...
export default function VerificationModal({
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      className={
        " w-4/12 p-10 rounded-xl h-1/3 min-h-80 min-w-96 bg-black/90 text-white outline-double absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 " +
    ...
    >
      <div className="w-full h-full flex flex-col text-center justify-around">
        <p className="text-5xl font-bold mt-2 animate-bouncepulse">
          Verification
        </p>
        <p className="text-xl  mt-2 ">
          Enter the verification code that was sent to{" "}
          <p className="font-bold animate-colorpulse">{email}</p>
        </p>
        <input
          type="tel"
          value={code}
