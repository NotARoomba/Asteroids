...
export default function EditModal({ isOpen, setIsOpen }: BaseModalProps) {
  ...
  return (
    <Modal
      ...
      isOpen={isOpen}
      className={
        " w-4/12 p-10 rounded-xl h-7/12 min-h-96 min-w-96 bg-black outline-double text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 " +
    ...
    >
      <div className="w-full h-full flex flex-col text-center ">
        <p className="text-5xl font-bold mb-4 animate-bouncepulse">
          Edit Profile
        </p>
        <div
        ...
        >
          {avatar !== "" ? (
            <img
              src={avatar}
              className="rounded-xl outline-double max-w-fit max-h-32 h-fit group"
            />
          ) : (
            <User size={125} />
          )}
        ...
        <div className="mx-auto mt-4">
          <p className="text-2xl font-bold animate-colorpulse">Username</p>
          <input
            value={username}
            onChange={(e) => setUsername(e.currentTarget.value)}
            className="mx-auto my-2 bg-transparent text-center outline rounded outline-primary"
          />
          <p className="text-2xl font-bold animate-colorpulse">Email</p>
          <input
            value={email}
